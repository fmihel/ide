...
RESOURCE('plugins','hint/example.dcss');
...
RESOURCE('plugins','hint/hint.js');
RESOURCE('plugins','hint/hint_define.dcss');
RESOURCE('plugins','hint/hint.dcss');

require_once UNIT('utils','framet.php');
RESOURCE('plugins','common/framet.js');
RESOURCE('https://code.jquery.com/ui/1.12.1/jquery-ui.js');
...
class TWS extends WS{
    ...
    public function plugin(){
        ...
        ->STYLE('border:1px solid black; background:gray;text-align:center;line-height:48px;color:white')
        ->SCRIPT('var tt = true;')
        ->ALIGN('if (tt) JX.pos({$},{x:20,y:20,w:230,h:48}); tt = false;')
        ->VALUE('plugin')
        ->READY('
            {$}.draggable();
        ');
        //->INIT('{$}.hint({})'); // place code for plugin init
    ...
    private function menu(){
        ...
        $this->item(
            'quickStart',                   //id
            'Quick start',                  //caption
            '$("#plugin").hint("Text in msg...");',//sample code
            '{$plugin}.hint("Text in msg");'    //onclick event
        );        
